...
function isSameLead(a: Lead, b: Lead): boolean {
  ...
  if (a.phone && b.phone && a.phone !== 'N/A' && b.phone !== 'N/A') {
    ...
    if (cleanPhoneA.slice(-10) === cleanPhoneB.slice(-10) && cleanPhoneA.length >= 10 && cleanPhoneB.length >= 10) {
      return true;
    }
  }

  // 3. Match by name + address if available on both
  if (a.fullName && b.fullName && a.address && b.address && a.address !== 'N/A' && b.address !== 'N/A') {
...
export function writeLeads(filePath: string, newLeads: Lead[]): void {
  ...
  const uniqueNewLeads = newLeads.filter(newLead => {
    const exists = existingData.some(existingLead => isSameLead(existingLead, newLead));
    if (exists) {
  ...
  console.log(`Total ${mergedData.length} results saved to ${filePath}. (Skipped ${newLeads.length - uniqueNewLeads.length} duplicates)`);
}